...
def polar_express(G: Tensor) -> Tensor:
    ...
    for a, b, c in [
        ...
        a = a / (1 + deflation_eps)
        b = b / (1 + deflation_eps)**3
        c = c / (1 + deflation_eps)**5
        A = X @ X.mT
...
def update(acc_bf16_view_u16: Tensor, mantissa: Tensor, momentum_buffer: Tensor, grad: Tensor, momentum: Tensor, eff_lr: Tensor, eff_weight_decay: Tensor):
    ...
    v =  mat_sign_function(momentum * momentum_buffer + (1 - momentum) * grad)
    # zeropower_via_newtonschulz5(momentum * momentum_buffer + (1 - momentum) * grad)
...
# begin logging
if master_process:
    run_id_full = f"{run_id:03d}_{uuid.uuid4()}" if run_name is None else run_name
